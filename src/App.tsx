...
function ChromeBar({ icon = '', label, onClose }: { icon?: string; label: string; onClose?: () => void }) {
  return (
    <div style={{ height: 22, flexShrink: 0, background: `linear-gradient(to right, ${ACCENT_DARK} 0%, ${ACCENT} 100%)`, display: 'flex', alignItems: 'center', padding: '0 4px 0 6px', gap: 5 }}>
      {icon && <span style={{ fontSize: 13 }}>{icon}</span>}
      <span style={{ fontFamily: CHROME, fontSize: 11, fontWeight: 700, color: OFF_WHITE, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{label}</span>
      {onClose ? (
        <button onClick={onClose} style={{ ...raised, width: 16, height: 14, background: SILVER, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'default', padding: 0, fontSize: 8, fontFamily: CHROME, fontWeight: 'bold', color: ACCENT_DARK, flexShrink: 0 }}>✕</button>
      ) : (
        <div style={{ display: 'flex', gap: 3 }}>
          {[0, 1, 2].map(i => <div key={i} style={{ width: 8, height: 8, background: SILVER, ...raisedSm }} />)}
        </div>
      )}
    </div>
  )
...
function ProfileModal({ onClose }: { onClose: () => void }) {
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9000, background: 'rgba(0,0,0,0.72)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ ...raised, background: SILVER, width: '100%', maxWidth: 680, display: 'flex', flexDirection: 'column', boxShadow: '8px 12px 40px rgba(0,0,0,0.7)', maxHeight: '90vh', overflow: 'hidden' }}>
        <ChromeBar icon="👤" label="Will Wingrove — Profile.exe" onClose={onClose} />
        <div style={{ display: 'flex', overflow: 'auto', background: OFF_WHITE }}>
          <div style={{ width: 220, flexShrink: 0, background: PAGE_DARK, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '32px 20px 24px', gap: 16 }}>
            <div style={{ width: 140, height: 140, ...sunken, overflow: 'hidden', background: '#2a2a2a', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ flex: 1, padding: '24px 28px 28px', display: 'flex', flexDirection: 'column', gap: 20, overflowY: 'auto' }}>
            <div>
              ...
              <p style={{ fontFamily: BODY, fontSize: 13, color: '#2a2a2a', lineHeight: 1.8, margin: '0 0 12px' }}>
              ...
              <p style={{ fontFamily: BODY, fontSize: 13, color: '#555', lineHeight: 1.8, margin: 0 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[
                  ...
                  <div key={val} style={{ ...raisedSm, background: LIGHT, padding: '6px 10px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 11, flexShrink: 0 }}>{label}</span>
                    <span style={{ fontFamily: BODY, fontSize: 12, color: '#333' }}>{val}</span>
                  </div>
                ))}
              </div>
            </div>
...
function ProfileHero({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <div onClick={onOpen} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display: 'flex', alignItems: 'center', cursor: 'default', marginBottom: 40, ...raised, background: SILVER, overflow: 'hidden' }}
    >
      <div style={{ width: 120, height: 120, flexShrink: 0, background: '#2a2a2a', position: 'relative', ...sunken, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ flex: 1, padding: '14px 20px' }}>
        <div style={{ fontFamily: CHROME, fontSize: 9, color: MID_GRAY, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Industrial Design Student · Carleton University</div>
        <div style={{ fontFamily: BODY, fontSize: 20, fontWeight: 600, color: BLACK, letterSpacing: '-0.02em', marginBottom: 6 }}>Will Wingrove</div>
        <div style={{ fontFamily: BODY, fontSize: 12, color: '#555', lineHeight: 1.6 }}>Ottawa, CA · Greenwich, CT · Fourth Year ID</div>
      </div>
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, borderLeft: `2px solid ${hovered ? ACCENT : DARK_GRAY}`, transition: 'border-color 0.15s' }}>
        ...
        <div style={{ fontFamily: CHROME, fontSize: 9, color: hovered ? ACCENT : MID_GRAY, transition: 'color 0.15s' }}>click to open</div>
      </div>
    </div>
  )
}
    ...
    <div style={{ ...raised, background: SILVER, display: 'flex', flexDirection: 'column' }}>
      ...
      <div onClick={onOpen} onMouseEnter={() => setBoardHovered(true)} onMouseLeave={() => setBoardHovered(false)} style={{ position: 'relative', cursor: 'pointer' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: boardHovered ? 'rgba(194,94,42,0.12)' : 'transparent', transition: 'background 0.15s', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
          {boardHovered && (
            <div style={{ ...raised, background: ACCENT, padding: '7px 18px', fontFamily: CHROME, fontSize: 11, fontWeight: 700, color: OFF_WHITE, letterSpacing: '0.04em' }}>
      ...
      <div style={{ display: 'flex', borderBottom: `1px solid ${DARK_GRAY}`, background: LIGHT, padding: '4px 4px 0' }}>
        {tabs.map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)} style={{ ...(tab === key ? sunken : raised), background: tab === key ? OFF_WHITE : SILVER, fontFamily: CHROME, fontSize: 11, color: tab === key ? ACCENT_DARK : '#333', fontWeight: tab === key ? 700 : 400, padding: '3px 14px', cursor: 'default', marginRight: 3, position: 'relative', top: tab === key ? 1 : 0 }}>{label}</button>
        ))}
      </div>
      <div style={{ background: OFF_WHITE, flex: 1, padding: 16 }}>
        {tab === 'overview' && (
          <div>
            <h3 style={{ fontFamily: BODY, fontSize: 16, fontWeight: 600, color: BLACK, margin: '0 0 4px', letterSpacing: '-0.02em' }}>{project.title}</h3>
            <p style={{ fontFamily: CHROME, fontSize: 9, fontWeight: 700, color: MID_GRAY, margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{project.subtitle}</p>
            <p style={{ fontFamily: BODY, fontSize: 13, color: '#888', lineHeight: 1.75, margin: 0, fontStyle: 'italic' }}>{project.summary}</p>
          </div>
        )}
        {tab === 'process' && (
...
function Nav() {
  return (
    <nav style={{ position: 'sticky', top: 0, zIndex: 100, background: SILVER, borderBottom: `2px solid ${DARK_GRAY}`, borderTop: `2px solid ${OFF_WHITE}`, boxSizing: 'border-box', display: 'flex', alignItems: 'center', padding: '0 40px', height: 40, gap: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, ...raisedSm, background: PAGE_DARK, padding: '3px 10px', marginRight: 20 }}>
        <div style={{ width: 7, height: 7, background: ACCENT }} />
        <span style={{ fontFamily: CHROME, fontSize: 11, fontWeight: 700, color: OFF_WHITE, letterSpacing: '0.08em' }}>Will Wingrove</span>
      </div>
      {['Work', 'About', 'Contact'].map(item => (
        <a key={item} href={`#${item.toLowerCase()}`} style={{ fontFamily: CHROME, fontSize: 11, color: '#333', padding: '4px 12px', textDecoration: 'none', cursor: 'default' }}
...
function Hero() {
  return (
    <section style={{ background: PAGE_DARK, padding: '120px 80px 100px', position: 'relative', overflow: 'hidden' }}>
...
function ProjectsSection() {
  ...
  return (
    <>
      ...
      <section id="work" style={{ background: PAGE_DARK, padding: '80px 80px 100px' }}>
...
function AboutSection() {
  return (
    <section id="about" style={{ background: LIGHT, padding: '80px 80px 100px' }}>
      <SectionLabel>About</SectionLabel>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'start' }}>
        <div style={{ ...raised, background: SILVER, padding: 0, overflow: 'hidden' }}>
          <ChromeBar icon="📄" label="about.txt — Notepad" />
          <div style={{ padding: '20px 22px 24px', background: OFF_WHITE }}>
            <p style={{ fontFamily: BODY, fontSize: 14, color: '#222', lineHeight: 1.8, margin: '0 0 16px' }}>
              Fourth year Industrial Design student at Carleton University in Ottawa, Canada. Passionate about applying my skills to create innovative products.
            </p>
            <p style={{ fontFamily: BODY, fontSize: 14, color: '#444', lineHeight: 1.8, margin: 0 }}>
              Outside of school, you can find me in and around Greenwich, CT bass fishing, boating, stringing lacrosse sticks, and working as a lifeguard and bartender.
            </p>
          </div>
        ...
        <div style={{ ...raised, background: SILVER, padding: 0, overflow: 'hidden' }}>
          ...
          <div style={{ background: SILVER, padding: '10px 8px' }}>
            ...
            <div style={{ background: OFF_WHITE, ...sunken, padding: 0 }}>
              {SKILLS.map((s, i) => (
                <div key={s.label} style={{ display: 'flex', alignItems: 'center', padding: '6px 10px', background: i % 2 === 0 ? OFF_WHITE : LIGHT, borderBottom: '1px solid #e0e0e0', gap: 8 }}>
                  <span style={{ fontFamily: CHROME, fontSize: 10, color: '#555', width: 160, flexShrink: 0 }}>{s.label}</span>
                  <div style={{ flex: 1, height: 12, ...sunken, background: LIGHT, position: 'relative', overflow: 'hidden' }}>
                  ...
                  <span style={{ fontFamily: CHROME, fontSize: 9, color: ACCENT_DARK, width: 76, flexShrink: 0 }}>{s.value}</span>
                </div>
              ))}
            </div>
          </div>
...
function ContactFooter() {
  return (
    <footer id="contact" style={{ background: PAGE_DARK, padding: '80px 80px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 20, marginBottom: 80, alignItems: 'flex-start' }}>
        <div style={{ ...raised, background: SILVER, flex: 1, padding: 0, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '20px 20px 24px', background: OFF_WHITE }}>
            <p style={{ fontFamily: BODY, fontSize: 14, color: '#444', lineHeight: 1.7, margin: '0 0 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                ...
                <div key={val} style={{ ...raised, background: SILVER, padding: '6px 12px', display: 'flex', gap: 12, alignItems: 'center' }}>
                  <span style={{ fontFamily: CHROME, fontSize: 10, color: ACCENT_DARK, flexShrink: 0, width: 80 }}>{label}</span>
                  <span style={{ fontFamily: BODY, fontSize: 13, color: '#333' }}>{val}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ width: 240, display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[{ label: 'University', value: 'Carleton University' }, { label: 'Program', value: 'Industrial Design' }, { label: 'Year', value: 'Fourth Year' }, { label: 'Location', value: 'Ottawa, CA · Greenwich, CT' }].map(({ label, value }) => (
            <div key={label} style={{ ...raised, background: SILVER, padding: '8px 12px' }}>
      ...
      <div style={{ borderTop: '2px solid #2a2a2a', background: '#1a1a1a', display: 'flex', alignItems: 'center', height: 26 }}>
        ...
        <div style={{ flex: 1 }} />
        <div style={{ ...raisedSm, background: '#222', padding: '0 12px', height: '100%', display: 'flex', alignItems: 'center', gap: 8, fontFamily: CHROME, fontSize: 10, color: MID_GRAY }}>
...
export default function App() {
  return (
    <div style={{ minHeight: '100vh', background: PAGE_DARK, overflowX: 'hidden' }}>
      <Nav />
      <Hero />
      <ProjectsSection />
      <AboutSection />
